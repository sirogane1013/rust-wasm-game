...
pub async fn fetch_array_buffer(resource: &str) -> Result<ArrayBuffer> {
    let array_buffer = fetch_response(resource)
        .await?
        .array_buffer()
        .map_err(|e| anyhow!("Error loading array buffer {:#?}", e))?;

    JsFuture::from(array_buffer)
        .await
        .map_err(|e| anyhow!("Error converting array buffer into a future {:#?}", e))?
        .dyn_into()
        .map_err(|e| anyhow!("Error converting raw JSValue to ArrayBuffer {:#?}", e))
}
    ...
    document()
        .and_then(|doc| {
            doc.get_element_by_id(id)
                .ok_or_else(|| anyhow!("Element withd id {} not found", id))
        })
        .and_then(|element| {
            element
